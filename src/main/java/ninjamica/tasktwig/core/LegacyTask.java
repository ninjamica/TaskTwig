...
public class LegacyTask {
    ...
    private final ObservableList<LegacyTask> children = FXCollections.observableArrayList();
    private boolean hasChildren = false;
    @JsonInclude(value=JsonInclude.Include.NON_DEFAULT)
    private final BooleanProperty expanded = new SimpleBooleanProperty();


    public LegacyTask(String name, @Nullable TaskCategory category, TaskInterval interval, @Nullable LocalTime dueTime, int priority, List<LegacyTask> children, boolean expanded) {
        this(name, category, interval, dueTime, priority);
        this.children.addAll(children);
        this.expanded.set(expanded);
    }

    public LegacyTask(String name, @Nullable TaskCategory category, TaskInterval interval, @Nullable LocalTime dueTime, int priority) {
        this.name.set(name);
    ...
    public LegacyTask(String name, TaskInterval interval, int priority) {
        this(name, null, interval, null, priority);
    }

    public LegacyTask(JsonNode node, int version) {
        this(node, version, true);
    }

    public LegacyTask(JsonNode node, int version, boolean isParent) {
        String name;
        TaskCategory category = null;
        TaskInterval interval;
        LocalTime dueTime = null;
        int priority = 0;
        List<LegacyTask> children = new ArrayList<>();
            ...
            case 3, 4, 5, 6:
                ...
                if (childrenNode != null) {
                    for (JsonNode child : childrenNode) {
                        children.add(new LegacyTask(child, version, false));
                    }
                }
                break;

            default:
                System.err.println("Unsupported LegacyTask version: " + version);
                throw new TaskTwig.TwigJsonVersionException("Unsupported LegacyTask version: " + version);
        }

        this(name, category, interval, dueTime, priority, children, expanded);
    }

    public static void parseDataFile(JsonParser parser, List<LegacyTask> taskList, List<TaskCategory> categoryList) {
        parser.nextToken();
...
//                categoryList.add(new TaskCategory("Other", Paint.valueOf("ffffff")));
        ...
        parser.nextToken();
        TaskTwig.parseJsonList(taskList, parser, node -> new LegacyTask(node, version));
...
//        }
    ...
    public ObservableList<LegacyTask> getChildren() {
        return children;
    }

    @JsonGetter("children")
    public List<LegacyTask> getChildrenJson() {
